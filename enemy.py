...
class Enemy(Hero):
    def __init__(self, ctx, name, enemy_type):
        super().__init__(ctx, name, enemy_type)
    ...
    def create_enemy(self):
        enemy = Enemy(self.ctx, self.name, self.type)
    ...
    def enemy_attack(self, hero):
        ...
        if enemy_damage == 0:
            ...
            enemy_attack = f"{self.name.capitalize()} hits you for {str(enemy_damage)} point of damage."
        else:
            enemy_attack = f"{self.name.capitalize()} hits you for {str(enemy_damage)} points of damage."
